import React from 'react'
import './css/po.css';

const OfficialDocs = () => {
    return (
        <div className="pages-height w-screen overflow-x-auto">

            <div className="m-5 bg-blue-200">
                <h1 className="tb-title m-3 bg-blue-200">
                    National Service Scheme Chhattisgarh Swami Vivekanand Technical
                </h1>
                <h1 className="tb-title m-5 bg-blue-200">
                    Announcements
                </h1>
            </div>

            <table className="GeneratedTable table-auto text-center md:w-full lg:h-full  .border-2 .border-black">
                <tr>
                    <th style={{ width: "50px" }}>S.No.</th>
                    <th style={{ width: "50px" }}>Date</th>
                    <th style={{ width: "50px" }}>Notice</th>
                    <th style={{ width: "50px" }}>Link</th>

                </tr>
                <tr>
                    <td style={{ width: "50px" }}></td>
                    <td style={{ width: "50px" }}></td>
                    <td style={{ width: "50px" }}></td>
                    <td style={{ width: "50px" }}></td>


                </tr>
                <tr>
                    <td style={{ width: "50px" }}>1</td>
                    <td style={{ width: "50px" }}>04-12-21</td>
                    <td style={{ width: "50px" }}>Regarding Aazadi Ka Amrit Mahotsav</td>
                    <td style={{ width: "50px" }}>

                            <a
                                href="https://drive.google.com/file/d/1IWVq7sPfdf4MYmllESVtOREJLCK1Dkw2/view?usp=sharing"
                                target="_blank"
                            >
                                <i className="fa fa-download" aria-hidden="true"></i>
                            </a></td>

                </tr>
                <tr>
                    <td style={{ width: "50px" }}>2</td>
                    <td style={{ width: "50px" }}>04-12-21</td>
                        <td style={{ width: "50px" }}>Regarding Free Vaccination Campaign</td>
                        <td style={{ width: "50px" }}> <a
                            href="https://drive.google.com/file/d/1Lu9K8kRmdU4cw5kD98mSCSm5m53P3mbf/view?usp=sharing"
                            target="_blank"
                        >
                            <i className="fa fa-download" aria-hidden="true"></i>
                        </a></td>

                </tr>
                <tr>
                    <td style={{ width: "50px" }}>3</td>
                    <td style={{ width: "50px" }}>04-12-21</td>
                    <td style={{ width: "50px" }}>In regards to the prevention and awareness of common non-communicable diseases
                    </td>

                    <td style={{ width: "50px" }}>
                        <a
                        href="https://drive.google.com/file/d/1uIYvoQ0bSEqK2Z9LI_7tDhKsBCpiReF-/view?usp=sharing"
                        target="_blank"
                    >
                        <i className="fa fa-download" aria-hidden="true"></i>
                    </a></td>

                </tr>
                <tr>
                    <td style={{ width: "50px" }}>4</td>
                    <td style={{ width: "50px" }}>26-11-21</td>
                    <td style={{ width: "50px" }}>SOCIAL MEDIA WORKSHOP :- A virtual mode online webinar regarding the use of social media for the promotion of activities through social media.</td>

                    <td style={{ width: "50px" }}> <a
                        href="https://drive.google.com/file/d/1YA9mSkdhXHHkvUSB2CcoLA4XDZ8Kcnks/view?usp=sharing"
                        target="_blank"
                    >
                        <i className="fa fa-download" aria-hidden="true"></i>
                    </a></td>

                </tr>
                <tr>
                    <td style={{ width: "50px" }}>5</td>
                    <td style={{ width: "50px" }}>08-11-2021 to 10-11-2021</td>
                    <td style={{ width: "50px" }}>National Service Scheme State Level 3 days 'Chintan Shivir'(meditation camp) at Prayog Ashram, Tilda, District Raipur C.G.</td>

                    <td style={{ width: "50px" }}> <a
                        href="https://drive.google.com/file/d/1N4lbuusPsE4OYRGWV5j1MCwL-vvvuA8E/view?usp=sharing"
                        target="_blank"
                    >
                        <i className="fa fa-download" aria-hidden="true"></i>
                    </a></td>

                </tr>



            </table>
        </div>
    )
}

export default OfficialDocs
